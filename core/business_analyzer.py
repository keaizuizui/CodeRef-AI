...
# ========================================================================
#  模块级辅助函数（从 BusinessAnalyzer 类提取；类内保留同名方法作薄委托）
#  分组：通用工具 → Stage1 实体 → Stage2 角色/工作流 → 层级化 →
#        Stage3 差异 → Stage4 评估 → Stage5 自改进 → 报告渲染
# ========================================================================

def _ba_sanitize_memory(memory: str, limit: int = 1000) -> str:
...
def _ba_call_llm(llm, prompt: str) -> str:
    """调用 LLM，支持重试与不可用占位（原 _call_llm 的模块级实现）"""
...
def _ba_module_from_path(file_path: str) -> str:
...
def _ba_extract_code_sample(cf) -> str:
...
def _ba_dir_to_business_name(llm, dir_name: str, code_context: str = "") -> str:
    """将目录名转换为业务名称（LLM 驱动，无硬编码映射）

    当 LLM 不可用时，使用基本的文本清理作为降级方案。
    """
    # 如果有 LLM 且提供了代码上下文，用 LLM 推断
    if llm and hasattr(llm, 'chat_completion') and code_context:
        try:
...
def _ba_infer_capabilities_from_names(llm, names: List[str], code_context: str = "") -> List[str]:
...
def _ba_summarize_signals(llm, signals: List[str], dir_name: str, code_context: str = "") -> str:
...
def _ba_overview_file_block(f, analysis) -> List[str]:
    """为单个文件生成概览行（类/函数名 + docstring 提示）"""
...
def _ba_build_compact_overview(analysis) -> str:
    """构建紧凑的代码概览，用于 LLM 分析
    ...
    for dir_name, files in sorted(dir_groups.items()):
        ...
        for f in files[:8]:  # 每个目录只看前 8 个文件
            lines.extend(_ba_overview_file_block(f, analysis))
...
def _ba_parse_llm_entities(response: str) -> List[BusinessEntity]:
    """解析 LLM 返回的业务实体 JSON"""
    try:
...
def _ba_merge_entities(heuristic: List[BusinessEntity], llm: List[BusinessEntity]) -> List[BusinessEntity]:
...
def _ba_collect_entity_symbols(entity, analysis, enrichment) -> list:
    """收集实体文件对应的 GitNexus 符号（直接匹配 + 前后缀匹配）"""
    symbols_for_entity = []
    ...
    return symbols_for_entity


def _ba_apply_entity_symbols(entity, symbols_for_entity, llm) -> None:
    """把 GitNexus 符号信息补充到实体（核心类 + 能力推断）"""
    # 提取函数名/类名
    ...
    if not entity.capabilities and func_names:
        entity.capabilities = _ba_infer_capabilities_from_names(llm, func_names)[:6]


def _ba_enrich_entities_with_symbols(entities, enrichment, analysis, llm) -> List[BusinessEntity]:
    """用 GitNexus 符号数据增强实体（补充核心类/函数）"""
    for entity in entities:
        # 找该实体文件对应的 GitNexus 符号
        symbols_for_entity = _ba_collect_entity_symbols(entity, analysis, enrichment)
        if symbols_for_entity:
            _ba_apply_entity_symbols(entity, symbols_for_entity, llm)

    return entities


def _ba_module_roles_from_prompt(pa) -> Dict[str, List[str]]:
    """从 Prompt 分析结果构建 模块 → 角色标签列表 的映射"""
...
def _ba_enrich_entities_with_prompt_roles(
    entities: List[BusinessEntity], pa: PromptAnalysisResult
) -> List[BusinessEntity]:
    """用 Prompt 角色信息丰富实体描述
    ...
    """
    # 构建模块→角色列表的映射
    module_roles = _ba_module_roles_from_prompt(pa)
...
def _ba_heuristic_entity_discovery(analysis, llm, prompt_analysis) -> List[BusinessEntity]:
    ...
    for f in analysis.files:
        ...
        code_sample = _ba_extract_code_sample(f)
        if code_sample:
    ...
    for dir_name, data in sorted(top_dirs.items(), key=lambda x: -len(x[1]["files"])):
        ...
        class_signals = _ba_infer_capabilities_from_names(
            llm, data["classes"], code_context=code_context
        )
        func_signals = _ba_infer_capabilities_from_names(
            llm, data["funcs"], code_context=code_context
        )

        purpose_signals = class_signals + func_signals

        # 生成业务名称（LLM 驱动）
        business_name = _ba_dir_to_business_name(llm, dir_name, code_context=code_context)

        # 生成业务目的（LLM 驱动）
        purpose = _ba_summarize_signals(llm, purpose_signals, dir_name, code_context=code_context)
    ...
    if prompt_analysis and prompt_analysis.roles:
        entities = _ba_enrich_entities_with_prompt_roles(entities, prompt_analysis)

    return entities


def _ba_llm_entity_discovery(analysis, enrichment, llm, prompt_memory) -> List[BusinessEntity]:
    """用 LLM 深入发现业务实体"""
    # 准备紧凑的代码概览
    project_name = os.path.basename(analysis.project_path.rstrip('/\\').rstrip('\\'))
    overview = _ba_build_compact_overview(analysis)
    ...
    if prompt_memory:
        learning_section = (
            f"\n【上一轮分析的经验与改进方向】\n"
            f"{_ba_sanitize_memory(prompt_memory, 1500)}\n"
...
{{
  "entities": [
    ...
    response = _ba_call_llm(llm, prompt)
    entities = _ba_parse_llm_entities(response)
...
def _ba_roles_from_prompt_analysis(pa: PromptAnalysisResult) -> List[UserRole]:
...
# 角色线索关键词（按发现顺序使用，保持线索收集顺序稳定）
_ROLE_CLUE_KEYWORDS = ['role', '角色', 'admin', 'permission', 'user_type', 'access_level', 'A角色', 'B角色']


def _ba_role_clues_of_file(content: str, role_clues: list) -> None:
    """从单个文件内容中提取包含角色关键词的行（凑满上限即提前返回）"""
    for keyword in _ROLE_CLUE_KEYWORDS:
...
def _ba_collect_role_clues(analysis) -> List[str]:
    """扫描代码中角色相关的线索行（role/角色/admin/permission 等）"""
    role_clues = []
    for f in analysis.files:
        content = getattr(f, 'raw_content', '') or ''
        # 查找角色相关的代码模式
        _ba_role_clues_of_file(content, role_clues)
        if len(role_clues) >= 20:
            break
    return role_clues


def _ba_heuristic_role_discovery(analysis, llm) -> List[UserRole]:
    ...
    if llm and hasattr(llm, 'chat_completion'):
        # 扫描代码中角色相关的线索
        role_clues = _ba_collect_role_clues(analysis)
...
def _ba_llm_role_discovery(analysis, entities: List[BusinessEntity], llm, prompt_memory) -> List[UserRole]:
    """用 LLM 发现用户角色"""
    entity_desc = "\n".join(
    ...
    # 查看是否有配置文件包含角色信息
    config_hint = _ba_find_role_config(analysis)

    learning_section = ""
    if prompt_memory:
        learning_section = (
            f"\n【上一轮分析的经验】\n"
            f"{_ba_sanitize_memory(prompt_memory, 1000)}\n"
            f"请在角色分析中重点关注上述方向。\n"
        )
...
{{
  "roles": [
    ...
    response = _ba_call_llm(llm, prompt)
    return _ba_parse_llm_roles(response)


def _ba_find_role_config(analysis) -> str:
    """查找配置中的角色信息"""
    hints = []
    for f in analysis.files:
        content = getattr(f, 'raw_content', '') or ''
...
def _ba_parse_llm_roles(response: str) -> List[UserRole]:
    """解析 LLM 返回的角色 JSON"""
    try:
        json_match = re.search(r'\{.*\}', response, re.DOTALL)
        if not json_match:
...
def _ba_workflows_from_prompt_analysis(pa: PromptAnalysisResult) -> List[BusinessWorkflow]:
...
def _ba_kb_workflows(kb_analyzer) -> List[BusinessWorkflow]:
    """从知识库调用链提取工作流（异常时返回空并告警）"""
    workflows = []
    try:
...
def _ba_docstring_methods_of_file(f, entity_technical_name, rel) -> list:
    """收集实体对应文件中带 docstring 的公开方法/函数（名称 + 文档前 100 字符）"""
    if not (entity_technical_name in rel or entity_technical_name == os.path.dirname(rel)):
        return []
    methods_with_docs = []
    for cls in f.classes:
...
def _ba_docstring_workflows(analysis, entities) -> List[BusinessWorkflow]:
    """降级：从 docstring 和调用关系推断工作流（不依赖硬编码关键词）"""
    workflows = []
    for entity in entities:
        ...
        for f in analysis.files:
            rel = os.path.relpath(f.file_path, analysis.project_path)
            methods_with_docs.extend(
                _ba_docstring_methods_of_file(f, entity.technical_name, rel)
            )
...
def _ba_heuristic_workflow_discovery(analysis, entities, kb_analyzer, llm) -> List[BusinessWorkflow]:
    ...
    """
    # 优先使用知识库 + LLM 分析
    if kb_analyzer and llm and hasattr(llm, 'chat_completion'):
        workflows = _ba_kb_workflows(kb_analyzer)
        if workflows:
            return workflows

    # 降级：从 docstring 和调用关系推断（不依赖硬编码关键词）
    return _ba_docstring_workflows(analysis, entities)


def _ba_flow_clues_of_file(f, analysis) -> List[str]:
    """从单个文件提取工作流线索（流程相关类/函数 + docstring）"""
    clues = []
...
def _ba_extract_flow_clues(analysis) -> str:
    """从代码中提取工作流线索"""
    clues = []
    for f in analysis.files:
        clues.extend(_ba_flow_clues_of_file(f, analysis))

    return "\n".join(clues[:20]) if clues else "（未发现明确的工作流代码注释）"


def _ba_llm_workflow_discovery(analysis, entities, roles, enrichment, llm, prompt_memory) -> List[BusinessWorkflow]:
    """用 LLM 发现业务流程"""
    entity_desc = "\n".join(
    ...
    # 查找关键流程代码
    flow_clues = _ba_extract_flow_clues(analysis)
...
{{
  "workflows": [
    ...
    response = _ba_call_llm(llm, prompt)
    return _ba_parse_llm_workflows(response)


def _ba_parse_llm_workflows(response: str) -> List[BusinessWorkflow]:
    """解析 LLM 返回的工作流 JSON"""
    try:
        json_match = re.search(r'\{.*\}', response, re.DOTALL)
        if not json_match:
...
# ── 层级化（Stage 2.5）辅助：_hier_* 为调用链重建的步骤函数 ──

def _hier_func_maps(enrichment) -> Tuple[Dict[str, str], Dict[str, str]]:
    """构建 func_name → module_name / file_path 映射（优先 file_symbols）"""
    ...
    for file_path, symbols in enrichment.file_symbols.items():
        module = _ba_module_from_path(file_path)
        for s in symbols:
            name = s.get("name", "")
            if name:
    ...
    if not func_to_module and enrichment.all_symbols:
        for s in enrichment.all_symbols:
            ...
            if name and fp:
                module = _ba_module_from_path(fp)
                func_to_module[name] = module
                func_to_file[name] = fp

    return func_to_module, func_to_file


def _hier_edges_from_call_pairs(enrichment, func_to_module) -> list:
    """从 call_pairs 直接收集模块间调用边 (caller_mod, callee_mod)"""
    edges = []
    for caller, callee, rel_type in enrichment.call_pairs:
        ...
        if caller_mod and callee_mod and caller_mod != callee_mod:
            edges.append((caller_mod, callee_mod))
    return edges


def _hier_edges_from_caller_index(enrichment, func_to_module) -> list:
    """利用 caller_index（callee → [callers]）补充调用边"""
    edges = []
...
def _hier_edges_from_callee_index(enrichment, func_to_module) -> list:
    """利用 callee_index（caller → [callees]）补充调用边"""
    edges = []
    if enrichment.callee_index:
...
def _hier_module_graph(enrichment, func_to_module) -> Tuple[Dict[str, set], Dict[str, set], Dict]:
    """构建双向模块级调用图（outgoing/incoming/边计数，三个索引源合并）"""
    ...
    # 三个索引源统一归并为 (caller_mod, callee_mod) 边列表（重复边在计数上累加）
    edges = (
        _hier_edges_from_call_pairs(enrichment, func_to_module)
        + _hier_edges_from_caller_index(enrichment, func_to_module)
        + _hier_edges_from_callee_index(enrichment, func_to_module)
    )
    for caller_mod, callee_mod in edges:
    ...
    return module_outgoing, module_incoming, module_edge_count


def _hier_filter_modules(all_modules) -> set:
    """过滤非模块名（单个 .py 文件、测试/安装脚本等噪声）"""
...
def _hier_arch_signaled(arch_profile, project_path, all_modules) -> set:
    ...
    """
    if not (arch_profile and arch_profile.entry_signals):
        return set()
    signaled_modules = set()
    for sig in arch_profile.entry_signals:
        fp = sig.file_path
        # 静态探测的 file_path 是绝对路径，先转成项目根相对路径再取模块名
        if project_path:
            try:
                fp = os.path.relpath(fp, project_path)
            except Exception:
                # relpath 失败（跨盘符等），保留绝对路径
                pass
        m = _ba_module_from_path(fp)
        if m in all_modules:
            signaled_modules.add(m)
    return signaled_modules


def _hier_classify_modules(all_modules, module_outgoing, module_incoming,
                           out_degree, in_degree, arch_profile, project_path):
    """识别三层架构：入口层 / 业务层 / 基础设施层"""
    entry_modules = _hier_entry_modules(all_modules, out_degree, in_degree)
    infra_modules = _hier_infra_modules(all_modules, out_degree, in_degree)

    # 架构探测增强入口
    signaled = _hier_arch_signaled(arch_profile, project_path, all_modules)
    newly_added = signaled - entry_modules
    entry_modules |= signaled
    if newly_added:
        logger.info(f"[BusinessAnalyzer] 架构探测增强入口模块: "
                    f"{sorted(newly_added)}（架构={arch_profile.label}）")
    ...
    return entry_modules, infra_modules, business_modules


def _hier_pick_spine(entry_modules, business_modules, entities, out_degree):
    """确定主轴（入口模块中优先选 web，其次选出度最大的）"""
    spine_module = None
    if entry_modules:
...
def _hier_make_spine_node(spine_module, entities, out_degree) -> WorkflowHierarchy:
    """构建主轴节点（level=0, is_spine=True）"""
...
def _hier_sorted_level1(spine_module, business_modules, module_outgoing,
                        module_edge_count, in_degree) -> list:
    """一级子模块列表：被主轴调用或调用基础设施的模块，按调用次数降序"""
...
def _hier_make_level1_child(module_name, spine_module, entities, workflows) -> WorkflowHierarchy:
    """构建一级子节点并填充工作流步骤"""
...
def _hier_attach_level2(child, module_name, spine_module, module_outgoing,
                        module_edge_count, infra_modules, entities, workflows) -> None:
    """为一级节点挂接二级子节点（该模块调用的基础设施模块）"""
...
def _hier_attach_uncovered_infra(spine_node, spine_module, level1_modules,
                                 infra_modules, entities) -> None:
    """把未归入层级的基础设施模块作为独立节点挂在主轴下"""
    covered_modules = {spine_module} | set(level1_modules)
...
def _ba_build_hierarchy_from_callgraph(
    enrichment, entities: List[BusinessEntity],
    workflows: List[BusinessWorkflow], roles: List[UserRole],
    llm, arch_profile, project_path
    ...
    # ── Step 2: 构建双向模块级调用图 ──
    module_outgoing, module_incoming, module_edge_count = _hier_module_graph(
        enrichment, func_to_module
    )
    all_modules = _hier_filter_modules(
        set(list(module_outgoing.keys()) + list(module_incoming.keys()))
    )
    if not all_modules:
    ...
    entry_modules, infra_modules, business_modules = _hier_classify_modules(
        all_modules, module_outgoing, module_incoming,
        out_degree, in_degree, arch_profile, project_path
    )
    ...
    spine_module = _hier_pick_spine(entry_modules, business_modules, entities, out_degree)
    if not spine_module:
    ...
    # 层级结构：入口 → 业务 → 基础设施
    hierarchy = []
    spine_node = _hier_make_spine_node(spine_module, entities, out_degree)

    sorted_level1 = _hier_sorted_level1(
        spine_module, business_modules, module_outgoing, module_edge_count, in_degree
    )
    for module_name in sorted_level1:
        child = _hier_make_level1_child(module_name, spine_module, entities, workflows)
        _hier_attach_level2(
            child, module_name, spine_module, module_outgoing,
            module_edge_count, infra_modules, entities, workflows
        )
        spine_node.children.append(child)

    _hier_attach_uncovered_infra(spine_node, spine_module, sorted_level1, infra_modules, entities)
    hierarchy.append(spine_node)

    # ── Step 6: 用 LLM 丰富描述（如果有 LLM）──
    if llm and hasattr(llm, 'chat_completion'):
        try:
            hierarchy = _ba_enrich_hierarchy_with_llm(
                llm, hierarchy, spine_module, module_edge_count
            )
        except Exception as e:
...
def _ba_collect_nodes_to_enrich(hierarchy, module_call_count) -> list:
    """收集描述过短、需要 LLM 丰富的层级节点（主轴 + 一级）"""
    nodes_to_enrich = []
    for h in hierarchy:
    ...
    return nodes_to_enrich


def _ba_enrich_hierarchy_with_llm(
    llm, hierarchy: List[WorkflowHierarchy], spine_module: str,
    module_call_count: Dict[str, Dict[str, int]]
) -> List[WorkflowHierarchy]:
    """用 LLM 丰富层级节点的描述（不改变结构，只优化文字）"""
    # 收集需要丰富描述的节点
    nodes_to_enrich = _ba_collect_nodes_to_enrich(hierarchy, module_call_count)
...
def _ba_llm_hierarchical_workflows(
    workflows: List[BusinessWorkflow], entities: List[BusinessEntity],
    roles: List[UserRole], llm
) -> List[WorkflowHierarchy]:
    """用 LLM 推断工作流的层级关系"""
    # 构建供 LLM 分析的摘要
    workflow_info = []
...
- 如果工作流之间有明确的上下游关系，用 children 嵌套"""
    ...
    try:
        ...
        if json_match:
            ...
            return _ba_parse_hierarchy(data)
    except Exception as e:
        logger.warning(f"[BusinessAnalyzer] LLM 层级化解析失败: {e}")

    return _ba_rule_hierarchical_workflows(workflows, entities, roles)


def _ba_parse_hierarchy(data: list, level: int = 0) -> List[WorkflowHierarchy]:
    """递归解析层级 JSON"""
    result = []
    for item in data:
        ...
        if children:
            node.children = _ba_parse_hierarchy(children, level=node.level + 1)
        result.append(node)
    return result


def _ba_rule_spine_modules(entities) -> set:
    """规则降级：找主轴模块（有 Web 入口或主入口的）"""
    spine_modules = set()
    for e in entities:
    ...
    return spine_modules


def _ba_rule_module_node(module_name, wfs, entities, is_spine) -> WorkflowHierarchy:
    """规则降级：为单个模块构建层级节点（单工作流平铺 / 多工作流父子两层）"""
    entity = next((e for e in entities if e.technical_name == module_name), None)
...
def _ba_rule_hierarchical_workflows(
    workflows: List[BusinessWorkflow], entities: List[BusinessEntity],
    roles: List[UserRole]
) -> List[WorkflowHierarchy]:
    """规则降级：按模块名+角色关系推测层级
    ...
    spine_modules = _ba_rule_spine_modules(entities)
    ...
    for module_name, wfs in module_workflows.items():
        node = _ba_rule_module_node(module_name, wfs, entities, module_name in spine_modules)
        if module_name in spine_modules:
...
def _ba_collect_ui_clues(analysis) -> Tuple[List[str], bool, bool]:
    """收集 UI 相关文件线索，返回 (线索列表, 是否有GUI, 是否有Web)"""
    ui_clues = []
    has_gui = False
    has_web = False
    for f in analysis.files:
    ...
    return ui_clues, has_gui, has_web


def _ba_heuristic_difference_discovery(analysis, llm) -> List[CrossCuttingDifference]:
    ...
    if llm and hasattr(llm, 'chat_completion'):
        # 收集 UI 相关的文件线索
        ui_clues, has_gui, has_web = _ba_collect_ui_clues(analysis)
...
def _ba_llm_difference_discovery(analysis, entities, roles, enrichment, llm, prompt_memory) -> List[CrossCuttingDifference]:
    """用 LLM 发现差异"""
    entity_desc = "\n".join(f"- {e.name}" for e in entities[:8])
    role_desc = "\n".join(f"- {r.name} ({r.description[:50]})" for r in roles[:4])

    # 查找 GUI/Web 文件线索
    ui_clues = _ba_find_ui_clues(analysis)
...
{{
  "differences": [
    ...
    response = _ba_call_llm(llm, prompt)
    return _ba_parse_llm_differences(response)


def _ba_find_ui_clues(analysis) -> str:
    """查找 UI 相关线索"""
    clues = []
...
def _ba_parse_llm_differences(response: str) -> List[CrossCuttingDifference]:
    """解析LLM返回的差异JSON"""
    try:
        json_match = re.search(r'\{.*\}', response, re.DOTALL)
        if not json_match:
...
# ── Stage 4: 自评估辅助（各维度独立评分，保持原累加顺序） ──

def _eval_entity_quality(result) -> Tuple[float, Optional[str]]:
    """实体维度：已有的实体是否有清晰描述"""
    if not result.entities:
        return 0.0, "未发现任何业务实体"
    ...
    if good_ratio >= 0.7:
        return 1.0, None  # 大部分实体描述清晰
    if good_ratio >= 0.3:
        return 0.5, f"部分业务实体（{len(result.entities) - len(good_entities)}个）缺少详细描述"
    return 0.0, "业务实体描述过于笼统"


def _eval_role_quality(result) -> Tuple[float, Optional[str]]:
    """角色维度：已有的角色是否有描述（无角色不扣分）"""
    if not result.roles:
        # 不扣分——很多项目根本没有多角色概念
        return 0.8, None  # 中性处理：不扣分，也不给满分
    good_roles = [r for r in result.roles if len(r.description) > 5]
    if len(good_roles) == len(result.roles):
        return 1.0, None
    return 0.5, f"{len(result.roles) - len(good_roles)} 个角色描述不完整"
...
def _eval_workflow_quality(result) -> Tuple[float, Optional[str]]:
    ...
    if len(good_flows) == len(result.workflows):
        return 1.0, None
    if len(good_flows) > 0:
        return 0.5, "部分业务流程步骤描述不完整"
    return 0.0, "业务流程缺少步骤说明"


def _eval_diff_quality(result) -> Tuple[float, Optional[str]]:
    """差异维度：已有的差异是否说明清楚（无差异不扣分）"""
    ...
    if len(good_diffs) == len(result.differences):
        return 1.0, None
    return 0.5, "部分差异描述不完整"


def _ba_evaluate_quality(result: BusinessAnalysisResult) -> AnalysisEvaluation:
    """自我评估分析质量（V2 通用版）
    ...
    # 4 个内容维度（顺序与原实现一致，保证浮点累加结果不变）
    for scorer in (_eval_entity_quality, _eval_role_quality,
                   _eval_workflow_quality, _eval_diff_quality):
        points, issue = scorer(result)
        quality_points += points
        if issue:
            issues.append(issue)

    # 5. 报告覆盖率：报告覆盖了几个维度
    covered = sum(1 for x in (result.entities, result.roles,
                              result.workflows, result.differences) if x)
    quality_points += covered / 4  # 0.25 ~ 1.0

    # 最终得分 = 质量分 / 总分（原口径：4 个内容维度 + 覆盖率共 5 项检查，再 +1，分母为 6）
    total_checks = 5
    score = quality_points / (total_checks + 1)
    ...
    suggestions = _ba_generate_evaluation_suggestions(issues)
...
def _ba_generate_evaluation_suggestions(issues: List[str]) -> List[str]:
    """根据问题生成改进建议"""
    suggestion_map = {
...
def _ba_result_summaries(result: BusinessAnalysisResult) -> Tuple[str, str, str, str]:
    """构建当前分析结果的四类摘要（实体/角色/流程/差异）"""
    entity_summary = "\n".join(
    ...
    return entity_summary, role_summary, workflow_summary, diff_summary


def _ba_generate_improvement(llm, result: BusinessAnalysisResult,
    ...
    """
    # 构建当前分析摘要
    entity_summary, role_summary, workflow_summary, diff_summary = _ba_result_summaries(result)
...
[你的硬编码 Prompt 内容]"""
    ...
    try:
        improved_prompt = llm.chat_completion([
...
def _ba_init_result(project_analysis) -> BusinessAnalysisResult:
    """从 CodeAnalyzer 结果初始化 BusinessAnalysisResult 的基础字段"""
...
def _ba_detect_arch_profile(files):
    """Stage 0.25: 架构探测（独立于调用图的静态证据源，用于增强入口发现）"""
    try:
        profile = detect_architecture(files)
        if profile:
            logger.info(f"[BusinessAnalyzer] 架构探测: {profile.label} "
                        f"(置信度 {profile.confidence:.2f}, "
                        f"入口信号 {len(profile.entry_signals)})")
        return profile
    except Exception as e:
...
def _ba_snapshot_result(result: BusinessAnalysisResult, iteration: int) -> BusinessAnalysisResult:
    """深拷贝当前轮结果作为最优快照（各字段独立副本，互不影响后续迭代）"""
...
# ── 报告渲染辅助（to_business_report 的各章节渲染函数） ──

def _ba_append_h2(lines: List[str], title: str) -> None:
    """追加二级标题段落（空行 + ## 标题 + 空行）"""
    lines.append('')
...
def _ba_append_h3(lines: List[str], title: str) -> None:
    """追加三级标题段落"""
    lines.append(f'### {title}')
    lines.append('')


def _ba_filter_business_entities(entities: List[BusinessEntity]) -> List[BusinessEntity]:
    """过滤非业务实体（测试/根模块/缓存等）"""
...
def _render_ba_header(lines: List[str], result: BusinessAnalysisResult) -> None:
    """渲染报告头部（标题 + 元信息）"""
...
def _render_ba_architecture(lines: List[str], entities: List[BusinessEntity]) -> None:
    """渲染第一章：系统架构总览（Mermaid 全景图 + 分层说明）"""
    _ba_append_h2(lines, '一、系统架构总览')
...
def _render_ba_entity_flows(lines: List[str], entity: BusinessEntity, entity_wfs: List[BusinessWorkflow]) -> None:
    """渲染单个实体的流程图（最多 2 个 Mermaid 流程）"""
...
def _render_ba_modules(lines: List[str], result: BusinessAnalysisResult,
                       entities: List[BusinessEntity]) -> None:
    """渲染第二章：业务模块详解（每个实体一张定位表 + 流程图）"""
    _ba_append_h2(lines, '二、业务模块详解')

    for idx, entity in enumerate(entities, 1):
        entity_wfs = [wf for wf in result.workflows if wf.owner == entity.name or wf.owner == entity.technical_name]

        _ba_append_h3(lines, f'{idx}. {entity.name} `{entity.technical_name}`')
        ...
        # 如果有工作流，展示流程图
        _render_ba_entity_flows(lines, entity, entity_wfs)


def _render_ba_hierarchy_nodes(lines: List[str], result: BusinessAnalysisResult) -> None:
    """渲染 3.1：层级总览（主轴 → 子层级的文字/表格形式）"""
    ...
    for node in result.hierarchy:
        ...
        # 递归输出子层级
        if node.children:
            _render_hier_children(lines, node.children, indent_level=1)
    ...
    node_counter = [0]
    for node in result.hierarchy:
        _render_hier_mermaid(lines, node, "Start", node_counter)

    lines.append('```')
    lines.append('')


def _render_ba_flows(lines: List[str], result: BusinessAnalysisResult) -> None:
    """渲染第三章：核心业务流程（层级化 → 平面降级 → 空占位）"""
    _ba_append_h2(lines, '三、核心业务流程（按层级捋清楚）')

    if result.hierarchy:
        _render_ba_hierarchy_nodes(lines, result)
...
def _render_ba_roles(lines: List[str], result: BusinessAnalysisResult,
                     entities: List[BusinessEntity]) -> None:
    """渲染第四章：用户角色与权限（角色定义表 + 角色-功能矩阵）"""
    _ba_append_h2(lines, '四、用户角色与权限')
...
def _render_ba_call_relations(lines: List[str], mod_calls) -> None:
    """渲染 5.1：模块级调用关系矩阵（TOP 15）"""
...
def _render_ba_dependencies(lines: List[str], result: BusinessAnalysisResult) -> None:
    """渲染第五章：模块间依赖关系（调用矩阵 + 入口点分布）"""
    _ba_append_h2(lines, '五、模块间依赖关系')
    ...
    if result.enrichment and result.enrichment.available:
        ...
        _render_ba_call_relations(lines, mod_calls)
...
def _render_ba_differences(lines: List[str], result: BusinessAnalysisResult) -> None:
    """渲染第六章：跨端/跨角色差异"""
    _ba_append_h2(lines, '六、跨端与角色差异')
    ...
    if result.differences:
        ...
        lines.append('（未识别到显著的跨端或角色差异）')
        lines.append('')


def _ba_overall_grade(result: BusinessAnalysisResult, entities: List[BusinessEntity]) -> Tuple[int, str]:
    """计算报告总体评分与等级（与原实现公式一致）"""
    score = 70
...
def _render_ba_findings(lines: List[str], result: BusinessAnalysisResult,
                        entities: List[BusinessEntity]) -> None:
    """渲染第七章：关键发现与建议（评分 + 架构优势 + 改进建议）"""
    _ba_append_h2(lines, '七、关键发现与建议')

    # 计算评分
    score, grade = _ba_overall_grade(result, entities)
...
def _render_hier_children(lines: List[str], children: List[WorkflowHierarchy], indent_level: int = 1) -> None:
    """递归渲染层级子节点"""
    indent = '  ' * indent_level
    ...
    for child in children:
        ...
        if child.children:
            _render_hier_children(lines, child.children, indent_level + 1)
        lines.append('')


def _render_hier_mermaid(lines: List[str], node: WorkflowHierarchy, parent_id: str, counter: List[int]) -> None:
    """渲染层级 Mermaid 节点"""
    counter[0] += 1
    ...
    for child in node.children:
        _render_hier_mermaid(lines, child, node_id, counter)


def _ba_call_summary_parts(callee_index) -> List[str]:
    """从 GitNexus callee_index 提取调用 TOP15 与跨模块调用 TOP10 摘要行"""
    parts = []
    # GitNexus 调用链 TOP 引用
    call_count = {}
    for caller, callees in callee_index.items():
...
def _ba_build_report_data_summary(result: BusinessAnalysisResult) -> str:
    ...
    if result.enrichment and result.enrichment.available and result.enrichment.callee_index:
        ...
        parts.extend(_ba_call_summary_parts(result.enrichment.callee_index))
...
class BusinessAnalyzer:
    ...
    def _sanitize_prompt_memory(memory: str, limit: int=1000) -> str:
        """把自学习记忆当作「不可信数据」安全注入 prompt，防提示注入。"""
        return _ba_sanitize_memory(memory, limit)
    ...
    def analyze(self, project_analysis, max_iterations=3) -> BusinessAnalysisResult:
        ...
        result = _ba_init_result(project_analysis)
        ...
        self._arch_profile = _ba_detect_arch_profile(project_analysis.files)
        ...
        while iteration < max_iterations:
            ...
            if evaluation.score > best_score:
                ...
                best_result = _ba_snapshot_result(result, iteration)
    ...
    def _enrich_entities_with_symbols(self, entities, enrichment, analysis) -> List[BusinessEntity]:
        """用 GitNexus 符号数据增强实体（补充核心类/函数）。"""
        return _ba_enrich_entities_with_symbols(entities, enrichment, analysis, self.llm)
    
    def _heuristic_entity_discovery(self, analysis) -> List[BusinessEntity]:
        """启发式实体发现（目录/命名结构推断，Prompt 角色优先）。"""
        return _ba_heuristic_entity_discovery(analysis, self.llm, self._prompt_analysis)

    def _extract_code_sample(self, cf) -> str:
        """抽取代码样本（类/函数签名片段，供 LLM 分析）。"""
        return _ba_extract_code_sample(cf)

    def _enrich_entities_with_prompt_roles(self, entities: List[BusinessEntity], pa: PromptAnalysisResult) -> List[BusinessEntity]:
        """用 Prompt 角色分析结果增强实体能力描述。"""
        return _ba_enrich_entities_with_prompt_roles(entities, pa)
    
    def _dir_to_business_name(self, dir_name: str, code_context: str='') -> str:
        """把目录名翻译成通俗的业务名称。"""
        return _ba_dir_to_business_name(self.llm, dir_name, code_context)
    
    def _infer_capabilities_from_names(self, names: List[str], code_context: str='') -> List[str]:
        """从类/函数命名推断业务能力描述。"""
        return _ba_infer_capabilities_from_names(self.llm, names, code_context)
    
    def _summarize_signals(self, signals: List[str], dir_name: str, code_context: str='') -> str:
        """汇总目录结构信号为一句业务描述。"""
        return _ba_summarize_signals(self.llm, signals, dir_name, code_context)
    
    def _llm_entity_discovery(self, analysis, enrichment=None) -> List[BusinessEntity]:
        """用 LLM 深入发现业务实体。"""
        return _ba_llm_entity_discovery(analysis, enrichment, self.llm, self._prompt_memory)
    
    def _build_compact_overview(self, analysis) -> str:
        """构建供 LLM 分析的紧凑代码概览（目录树 + 关键文件）。"""
        return _ba_build_compact_overview(analysis)
    
    def _parse_llm_entities(self, response: str) -> List[BusinessEntity]:
        """解析 LLM 返回的业务实体 JSON（含截断容错）。"""
        return _ba_parse_llm_entities(response)
    
    def _merge_entities(self, heuristic: List[BusinessEntity], llm: List[BusinessEntity]) -> List[BusinessEntity]:
        """合并启发式与 LLM 两路实体发现结果。"""
        return _ba_merge_entities(heuristic, llm)
    ...
    def _roles_from_prompt_analysis(self, pa: PromptAnalysisResult) -> List[UserRole]:
        """从 Prompt 分析结果提取用户角色。"""
        return _ba_roles_from_prompt_analysis(pa)
    
    def _heuristic_role_discovery(self, analysis) -> List[UserRole]:
        """启发式角色发现（配置/命名/Prompt 角色模式）。"""
        return _ba_heuristic_role_discovery(analysis, self.llm)
    
    def _llm_role_discovery(self, analysis, entities: List[BusinessEntity]) -> List[UserRole]:
        """用 LLM 发现用户角色。"""
        return _ba_llm_role_discovery(analysis, entities, self.llm, self._prompt_memory)
    
    def _find_role_config(self, analysis) -> str:
        """查找项目中的角色/权限配置文件内容。"""
        return _ba_find_role_config(analysis)
    
    def _parse_llm_roles(self, response: str) -> List[UserRole]:
        """解析 LLM 返回的角色 JSON（含截断容错）。"""
        return _ba_parse_llm_roles(response)
    ...
    def _build_hierarchy_from_callgraph(self, enrichment, entities: List[BusinessEntity], workflows: List[BusinessWorkflow], roles: List[UserRole]) -> List[WorkflowHierarchy]:
        """基于调用图构建层级工作流（主轴 → 一级 → 二级 → 角色步骤）。"""
        return _ba_build_hierarchy_from_callgraph(enrichment, entities, workflows, roles, self.llm, self._arch_profile, self._project_path)
    
    def _extract_module_from_path(self, file_path: str) -> str:
        """从文件路径提取模块名（项目根目录下的第一层）。"""
        return _ba_module_from_path(file_path)
    
    def _enrich_hierarchy_with_llm(self, hierarchy: List[WorkflowHierarchy], spine_module: str, module_call_count: Dict[str, Dict[str, int]]) -> List[WorkflowHierarchy]:
        """用 LLM 增强层级节点的业务语义描述。"""
        return _ba_enrich_hierarchy_with_llm(self.llm, hierarchy, spine_module, module_call_count)
    
    def _llm_hierarchical_workflows(self, workflows: List[BusinessWorkflow], entities: List[BusinessEntity], roles: List[UserRole]) -> List[WorkflowHierarchy]:
        """用 LLM 重建层级化工作流。"""
        return _ba_llm_hierarchical_workflows(workflows, entities, roles, self.llm)
    
    def _parse_hierarchy(self, data: list, level: int=0) -> List[WorkflowHierarchy]:
        """递归解析 LLM 返回的层级 JSON。"""
        return _ba_parse_hierarchy(data, level)
    
    def _rule_hierarchical_workflows(self, workflows: List[BusinessWorkflow], entities: List[BusinessEntity], roles: List[UserRole]) -> List[WorkflowHierarchy]:
        """规则式层级重建（无 LLM 时的降级方案）。"""
        return _ba_rule_hierarchical_workflows(workflows, entities, roles)
    
    def _workflows_from_prompt_analysis(self, pa: PromptAnalysisResult) -> List[BusinessWorkflow]:
        """从 Prompt 分析结果提取工作流。"""
        return _ba_workflows_from_prompt_analysis(pa)
    
    def _heuristic_workflow_discovery(self, analysis, entities) -> List[BusinessWorkflow]:
        """发现业务流程（知识库 + LLM 驱动，docstring 降级，无硬编码关键词）。"""
        return _ba_heuristic_workflow_discovery(analysis, entities, self._get_kb_analyzer(), self.llm)
    
    def _llm_workflow_discovery(self, analysis, entities, roles, enrichment=None) -> List[BusinessWorkflow]:
        """用 LLM 发现业务流程。"""
        return _ba_llm_workflow_discovery(analysis, entities, roles, enrichment, self.llm, self._prompt_memory)
    
    def _extract_flow_clues(self, analysis) -> str:
        """抽取流程线索（调用链/入口方法），供 LLM 流程分析。"""
        return _ba_extract_flow_clues(analysis)
    
    def _parse_llm_workflows(self, response: str) -> List[BusinessWorkflow]:
        """解析 LLM 返回的工作流 JSON（含截断容错）。"""
        return _ba_parse_llm_workflows(response)
    ...
    def _heuristic_difference_discovery(self, analysis) -> List[CrossCuttingDifference]:
        """启发式跨端差异发现（gui/web 目录与 UI 线索对比）。"""
        return _ba_heuristic_difference_discovery(analysis, self.llm)
    
    def _llm_difference_discovery(self, analysis, entities, roles, enrichment=None) -> List[CrossCuttingDifference]:
        """用 LLM 发现跨端/跨角色差异。"""
        return _ba_llm_difference_discovery(analysis, entities, roles, enrichment, self.llm, self._prompt_memory)
    
    def _find_ui_clues(self, analysis) -> str:
        """收集 UI 相关线索（界面代码/入口）。"""
        return _ba_find_ui_clues(analysis)
    
    def _parse_llm_differences(self, response: str) -> List[CrossCuttingDifference]:
        """解析 LLM 返回的差异 JSON（含截断容错）。"""
        return _ba_parse_llm_differences(response)
    ...
    def _evaluate_quality(self, result: BusinessAnalysisResult) -> AnalysisEvaluation:
        """自评估：按六大检查项给业务分析结果打分。"""
        return _ba_evaluate_quality(result)
    
    def _generate_evaluation_suggestions(self, issues: List[str]) -> List[str]:
        """针对评估发现的问题生成改进建议。"""
        return _ba_generate_evaluation_suggestions(issues)
    
    # ====================================================================
    #  Stage 5: 自改进
    # ====================================================================
    
    def _generate_improvement(self, result: BusinessAnalysisResult, evaluation: AnalysisEvaluation) -> str:
        """自改进：让 LLM 硬编码更好的分析 Prompt。"""
        return _ba_generate_improvement(self.llm, result, evaluation)
    ...
    def to_business_report(self, result: BusinessAnalysisResult) -> str:
        ...
        各章节渲染委托到模块级 _render_ba_* 函数，本方法只负责章节编排顺序：
        报告头 → 架构总览 → 模块详解 → 流程层级 → 角色权限 → 依赖关系 → 跨端差异 → 关键发现。
        """
        lines: List[str] = []
        entities = _ba_filter_business_entities(result.entities)

        # 一~四章：报告头 / 系统架构 / 业务模块 / 核心流程（层级或平面降级）
        _render_ba_header(lines, result)
        _render_ba_architecture(lines, entities)
        _render_ba_modules(lines, result, entities)
        _render_ba_flows(lines, result)

        # 五~七章：用户角色 / 模块依赖与调用关系 / 跨端差异
        _render_ba_roles(lines, result, entities)
        _render_ba_dependencies(lines, result)
        _render_ba_differences(lines, result)

        # 八章：关键发现与建议（评分 + 优势 + 改进建议）
        _render_ba_findings(lines, result, entities)
    ...
    def _build_report_data_summary(self, result: BusinessAnalysisResult) -> str:
        """生成报告数据摘要（供上级报告引用）。"""
        return _ba_build_report_data_summary(result)

    def _call_llm(self, prompt: str) -> str:
        """调用 LLM（含超时与临时错误重试，永久错误不重试）。"""
        return _ba_call_llm(self.llm, prompt)

    def _render_hierarchy_children(self, lines: List[str], children: List[WorkflowHierarchy], indent_level: int=1) -> None:
        """渲染层级子节点（缩进 Markdown 列表）。"""
        return _render_hier_children(lines, children, indent_level)

    def _render_hierarchy_mermaid(self, lines: List[str], node: WorkflowHierarchy, parent_id: str, counter: List[int]) -> None:
        """渲染层级节点的 mermaid 流程图。"""
        return _render_hier_mermaid(lines, node, parent_id, counter)
