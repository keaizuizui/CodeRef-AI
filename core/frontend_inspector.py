...
# 工具自产输出目录：其中的 HTML 是审计报告/历史快照模板，不是用户前端源码，
# 扫描与预检一律排除，避免把报告折叠按钮当作前端交互元素枚举
TOOL_OUTPUT_DIRS = {"coderef-report", ".gitnexus", "coderef-wiki"}
# 文档目录：其中的 HTML 通常是设计文档/导出文档（如 "Generated by Trae Work"
# 标记的设计文档页），非用户维护的前端交互源码，同样排除
DOC_DIRS = {"docs"}
IGNORED_DIRS = {".git", ".node_modules", "node_modules", "__pycache__", "dist", "build", ".venv", "venv"} | TOOL_OUTPUT_DIRS | DOC_DIRS

# 用户维护的前端源码扩展名（项目预检用）
FRONTEND_SOURCE_EXTS = (".html", ".htm", ".vue", ".jsx", ".tsx")
...
class ButtonItem:
    ...
    href: str = ""                                     # 跳转目标（跳转类元素）
    occurrences: int = 1                               # 同模板去重后的出现次数（>1 表示多份快照/多处重复）
...
class FrontendInspector:
    ...
    def _inspect_static(
        ...
        # 项目预检：确认存在用户维护的前端源码（HTML/Vue/JSX/TSX，已排除
        # coderef-report/.gitnexus 等工具自产输出目录）。纯 Python 项目的
        # HTML 通常只是工具自产的审计报告/历史快照，直接跳过静态枚举，
        # 避免把报告模板的折叠按钮当作前端交互元素产出大量误报 findings。
        if not self._has_user_frontend_sources(project_path, entry):
            logger.info("未检测到用户维护的前端源码，静态前端枚举已跳过")
            return {
                "ok": True,
                "applicable": False,
                "message": ("未检测到用户维护的前端源码（HTML/Vue/JSX/TSX），"
                            "该项目可能不是前端项目，静态前端枚举已跳过"),
            }
        ...
        for b in buttons:
            ...
            if key in _dedup:
                ...
                b.occurrences = 1
                _dedup[key] = b
                _dedup_buttons.append(b)
        buttons = _dedup_buttons
    ...
    # ── 文件发现与扫描 ──
    @staticmethod
    def _has_user_frontend_sources(project_path: Optional[str], entry: Optional[str]) -> bool:
        """预检项目内是否存在用户维护的前端源码（HTML/Vue/JSX/TSX）。

        排除 coderef-report/.gitnexus/coderef-wiki 等工具自产输出目录（IGNORED_DIRS
        已并入 TOOL_OUTPUT_DIRS）——这些目录里的 HTML 是审计报告/历史快照模板，
        不是用户前端代码。entry 语义与 _find_html_files 对齐：entry 为单个
        HTML 文件时直接以该文件判定；entry 为目录时预检该目录。
        """
        if entry and os.path.isfile(entry):
            return entry.lower().endswith((".html", ".htm"))
        scan_root = entry if entry and os.path.isdir(entry) else project_path
        if not scan_root or not os.path.isdir(scan_root):
            return False
        for _root, dirs, names in os.walk(scan_root):
            dirs[:] = [d for d in dirs if d not in IGNORED_DIRS]
            for name in names:
                if os.path.splitext(name)[1].lower() in FRONTEND_SOURCE_EXTS:
                    return True
        return False
