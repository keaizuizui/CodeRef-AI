...
class MemoryQuality:
    ...
    def _check_coverage(self, project_path: str, auto_fix: bool):
        ...
        if auto_fix and missing:
            ...
            for sym in missing:
                ...
                applied += 1
            self._save_auto_summary_store(project_path, store, root)
            # 标记已自动补全
    ...
    def _load_auto_summary_store(self, project_path: str) -> tuple:
        """加载并返回 (完整存储根, 该项目的条目容器)。"""
        ...
        # 显式返回 root 和 proj，避免依赖实例属性导致 save 时丢失其他项目数据
        return data, proj

    def _save_auto_summary_store(self, project_path: str, proj: Dict[str, Any],
                                 root: Dict[str, Any]):
        """把更新后的项目条目容器写回完整存储。root 为 loader 返回的完整存储根。"""
        data = root
        phash = hashlib.md5(project_path.encode("utf-8")).hexdigest()[:12]
    ...
    def _check_bias(self, project_path: str):
        ...
        if self.llm and hasattr(self.llm, "chat_completion"):
            try:
                ...
                # 无论 parsed 是否为空，LLM 已成功响应，直接返回（避免误报"无 LLM 可用"）
