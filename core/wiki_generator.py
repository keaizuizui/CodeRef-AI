...
# ==================== 模块级常量区（自 WikiGenerator 提取，配置与生成逻辑分离） ====================
# 大仓库阈值
...
def _whitelist_exclude_dirs(project_path: str) -> List[str]:
    """读取 whitelist 中的 dir 条目（目录级排除，备份/镜像目录不污染 wiki 生成）。
...
class WikiGenerator:
    ...
    # 此处保留同名类属性引用，兼容 WikiGenerator.XXX 与 self.XXX 的既有访问。
    LARGE_REPO_THRESHOLD = LARGE_REPO_THRESHOLD
    LARGE_REPO_SAMPLE = LARGE_REPO_SAMPLE
    MAX_FILES_PER_MODULE = MAX_FILES_PER_MODULE
    MAX_CONTEXT_CHARS = MAX_CONTEXT_CHARS
    MAX_ENTRY_DOCS = MAX_ENTRY_DOCS
    MAX_FLOW_DOCS = MAX_FLOW_DOCS
    DEFAULT_ENTRY_FILES = DEFAULT_ENTRY_FILES
    DEFAULT_MIN_FILES = DEFAULT_MIN_FILES
    CORE_RULES_FILE = CORE_RULES_FILE
    EXCLUDE_DIRS = EXCLUDE_DIRS
    SUBPROJECT_INDICATORS = SUBPROJECT_INDICATORS
    WIKI_STYLES = WIKI_STYLES
    METADATA_MAX_FILES = METADATA_MAX_FILES
    METADATA_MAX_CLASSES = METADATA_MAX_CLASSES
    METADATA_MAX_FUNCS = METADATA_MAX_FUNCS
    CITE_IGNORE = CITE_IGNORE
    ...
    @staticmethod
    def _core_rules_path(project_path: str):
        """拆分说明：实现已移至模块级 _core_rules_path，此处保留方法签名做委托"""
        return _core_rules_path(project_path)

    def _load_core_rules(self, project_path: str):
        """拆分说明：实现已移至模块级 _load_core_rules，此处保留方法签名做委托"""
        return _load_core_rules(self, project_path)

    @staticmethod
    def save_core_rules(project_path: str, rules: dict):
        """拆分说明：实现已移至模块级 save_core_rules，此处保留方法签名做委托"""
        return save_core_rules(project_path, rules)

    @staticmethod
    def get_core_rules(project_path: str):
    ...
    def _git_bin(self):
        ...
        return _git_bin(self)

    def _git_head(self, git_bin: str, project_path: str):
        """拆分说明：实现已移至模块级 _git_head，此处保留方法签名做委托"""
        return _git_head(self, git_bin, project_path)

    def _git_last_commit(self, git_bin: str, project_path: str, file_path: str):
        """拆分说明：实现已移至模块级 _git_last_commit，此处保留方法签名做委托"""
        return _git_last_commit(self, git_bin, project_path, file_path)

    def _git_changed_files(self, git_bin: str, project_path: str, last_head: str):
        """拆分说明：实现已移至模块级 _git_changed_files，此处保留方法签名做委托"""
        return _git_changed_files(self, git_bin, project_path, last_head)

    def _load_last_update(self, output_dir: str):
        """拆分说明：实现已移至模块级 _load_last_update，此处保留方法签名做委托"""
        return _load_last_update(self, output_dir)

    def _save_last_update(self, output_dir: str, git_head: Optional[str], doc_count: int):
    ...
    def _prune_stale_module_docs(self, output_dir: str, modules: List[WikiModule]):
        ...
        return _prune_stale_module_docs(self, output_dir, modules)

    def _discover_modules(self, project_path: str):
        """拆分说明：实现已移至模块级 _discover_modules，此处保留方法签名做委托"""
        return _discover_modules(self, project_path)

    def _collect_py_files(self, dir_path: str):
        """拆分说明：实现已移至模块级 _collect_py_files，此处保留方法签名做委托"""
        return _collect_py_files(self, dir_path)

    def _discover_subprojects(self, project_path: str):
        """拆分说明：实现已移至模块级 _discover_subprojects，此处保留方法签名做委托"""
        return _discover_subprojects(self, project_path)

    def _coderef_cache_dir(self):
        """拆分说明：实现已移至模块级 _coderef_cache_dir，此处保留方法签名做委托"""
        return _coderef_cache_dir(self)

    def _project_cache_path(self, project_path: str):
        """拆分说明：实现已移至模块级 _project_cache_path，此处保留方法签名做委托"""
        return _project_cache_path(self, project_path)

    def _build_code_metadata(self, modules: List[WikiModule], project_path: str, skip_cache: bool=False):
        """拆分说明：实现已移至模块级 _build_code_metadata，此处保留方法签名做委托"""
        return _build_code_metadata(self, modules, project_path, skip_cache)

    def _extract_file_metadata(self, content: str, rel_path: str):
        """拆分说明：实现已移至模块级 _extract_file_metadata，此处保留方法签名做委托"""
        return _extract_file_metadata(self, content, rel_path)

    def _metadata_to_dict(self, meta: ProjectCodeMetadata):
        """拆分说明：实现已移至模块级 _metadata_to_dict，此处保留方法签名做委托"""
        return _metadata_to_dict(self, meta)

    def _metadata_from_dict(self, d: Dict):
        """拆分说明：实现已移至模块级 _metadata_from_dict，此处保留方法签名做委托"""
        return _metadata_from_dict(self, d)

    def _sample_metadata(self, meta: ProjectCodeMetadata):
        """拆分说明：实现已移至模块级 _sample_metadata，此处保留方法签名做委托"""
        return _sample_metadata(self, meta)

    def _generate_module_descriptions(self, meta: ProjectCodeMetadata, style: str, invalidate: Optional[List[str]]=None):
        """拆分说明：实现已移至模块级 _generate_module_descriptions，此处保留方法签名做委托"""
        return _generate_module_descriptions(self, meta, style, invalidate)

    def _module_metadata_to_text(self, mod: ModuleCodeMetadata):
        """拆分说明：实现已移至模块级 _module_metadata_to_text，此处保留方法签名做委托"""
        return _module_metadata_to_text(self, mod)

    def _sample_large_repo(self, modules: List[WikiModule]):
        """拆分说明：实现已移至模块级 _sample_large_repo，此处保留方法签名做委托"""
        return _sample_large_repo(self, modules)

    def _collect_code_summaries(self, modules: List[WikiModule]):
        """拆分说明：实现已移至模块级 _collect_code_summaries，此处保留方法签名做委托"""
        return _collect_code_summaries(self, modules)

    def _extract_file_info(self, content: str, rel_path: str):
        """拆分说明：实现已移至模块级 _extract_file_info，此处保留方法签名做委托"""
        return _extract_file_info(self, content, rel_path)

    def _strip_deterministic_prefix(self, doc_text: str):
    ...
    def _generate_global_docs(self, project_name: str, modules: List[WikiModule], meta: ProjectCodeMetadata, descriptions: Dict[str, str], output_dir: str, result: WikiResult, cite_warnings: List[str]=None):
        ...
        return _generate_global_docs(self, project_name, modules, meta, descriptions, output_dir, result, cite_warnings)

    def _cross_verify_modules(self, project_path: str, modules: List[WikiModule], entry_spec: str):
        """拆分说明：实现已移至模块级 _cross_verify_modules，此处保留方法签名做委托"""
        return _cross_verify_modules(self, project_path, modules, entry_spec)

    def _build_project_summary(self, project_name: str, modules: List[WikiModule], summaries: Dict[str, str]):
        """拆分说明：实现已移至模块级 _build_project_summary，此处保留方法签名做委托"""
        return _build_project_summary(self, project_name, modules, summaries)

    def _load_instructions(self, project_path: str):
        """拆分说明：实现已移至模块级 _load_instructions，此处保留方法签名做委托"""
        return _load_instructions(self, project_path)

    def _instructions_text(self):
        """拆分说明：实现已移至模块级 _instructions_text，此处保留方法签名做委托"""
        return _instructions_text(self)

    def _llm_ask(self, system_prompt: str, user_prompt: str):
        """拆分说明：实现已移至模块级 _llm_ask，此处保留方法签名做委托"""
        return _llm_ask(self, system_prompt, user_prompt)

    def _style_guidelines(self, style: str):
        """拆分说明：实现已移至模块级 _style_guidelines，此处保留方法签名做委托"""
        return _style_guidelines(self, style)

    def _make_fact_constraint(self):
        """拆分说明：实现已移至模块级 _make_fact_constraint，此处保留方法签名做委托"""
        return _make_fact_constraint(self)

    def _extract_entry_files(self, meta: ProjectCodeMetadata):
        """拆分说明：实现已移至模块级 _extract_entry_files，此处保留方法签名做委托"""
        return _extract_entry_files(self, meta)

    def _generate_overview(self, project_name: str, modules: List[WikiModule], style: str='comprehensive', descriptions: Dict[str, str]=None, meta: ProjectCodeMetadata=None):
        """拆分说明：实现已移至模块级 _generate_overview，此处保留方法签名做委托"""
        return _generate_overview(self, project_name, modules, style, descriptions, meta)

    def _generate_readme(self, project_name: str, summary: str, modules: List[WikiModule], style: str='comprehensive', descriptions: Dict[str, str]=None, meta: ProjectCodeMetadata=None):
        """拆分说明：实现已移至模块级 _generate_readme，此处保留方法签名做委托"""
        return _generate_readme(self, project_name, summary, modules, style, descriptions, meta)

    def _generate_architecture(self, project_name: str, summary: str, modules: List[WikiModule], style: str='comprehensive', descriptions: Dict[str, str]=None, meta: ProjectCodeMetadata=None):
        """拆分说明：实现已移至模块级 _generate_architecture，此处保留方法签名做委托"""
        return _generate_architecture(self, project_name, summary, modules, style, descriptions, meta)

    def _generate_installation(self, project_name: str, summary: str, modules: List[WikiModule], style: str='comprehensive', meta: ProjectCodeMetadata=None):
        """拆分说明：实现已移至模块级 _generate_installation，此处保留方法签名做委托"""
        return _generate_installation(self, project_name, summary, modules, style, meta)

    def _generate_usage(self, project_name: str, summary: str, modules: List[WikiModule], style: str='comprehensive', descriptions: Dict[str, str]=None, meta: ProjectCodeMetadata=None):
    ...
    def _build_module_index(self, modules: List[WikiModule], cross_badges: Dict[str, dict]=None):
        ...
        return _build_module_index(self, modules, cross_badges)

    def _summarize_descriptions(self, descriptions: Dict[str, str]):
        """拆分说明：实现已移至模块级 _summarize_descriptions，此处保留方法签名做委托"""
        return _summarize_descriptions(self, descriptions)

    def _make_arch_overview(self, meta: ProjectCodeMetadata, descriptions: Dict[str, str]):
        """拆分说明：实现已移至模块级 _make_arch_overview，此处保留方法签名做委托"""
        return _make_arch_overview(self, meta, descriptions)

    def _locate_kg_db(self, project_path: str):
        """拆分说明：实现已移至模块级 _locate_kg_db，此处保留方法签名做委托"""
        return _locate_kg_db(self, project_path)

    @staticmethod
    def _sanitize_doc_name(name: str):
        """拆分说明：实现已移至模块级 _sanitize_doc_name，此处保留方法签名做委托"""
        return _sanitize_doc_name(name)

    def _discover_entry_points(self, meta: ProjectCodeMetadata, fv=None):
        """拆分说明：实现已移至模块级 _discover_entry_points，此处保留方法签名做委托"""
        return _discover_entry_points(self, meta, fv)

    def _extract_entry_chain(self, entry: dict, fv=None):
        """拆分说明：实现已移至模块级 _extract_entry_chain，此处保留方法签名做委托"""
        return _extract_entry_chain(self, entry, fv)

    def _generate_entry_doc(self, project_name: str, entry: dict, chain: List[dict], style: str, meta: ProjectCodeMetadata):
        """拆分说明：实现已移至模块级 _generate_entry_doc，此处保留方法签名做委托"""
        return _generate_entry_doc(self, project_name, entry, chain, style, meta)

    def _extract_cross_module_flows(self, fv=None):
        """拆分说明：实现已移至模块级 _extract_cross_module_flows，此处保留方法签名做委托"""
        return _extract_cross_module_flows(self, fv)

    def _generate_flow_doc(self, project_name: str, flow: dict, style: str):
        """拆分说明：实现已移至模块级 _generate_flow_doc，此处保留方法签名做委托"""
        return _generate_flow_doc(self, project_name, flow, style)

    def _make_web_info(self, meta: ProjectCodeMetadata):
        """拆分说明：实现已移至模块级 _make_web_info，此处保留方法签名做委托"""
        return _make_web_info(self, meta)

    def _cite_verify(self, text: str, meta: ProjectCodeMetadata, doc_name: str):
        """拆分说明：实现已移至模块级 _cite_verify，此处保留方法签名做委托"""
        return _cite_verify(self, text, meta, doc_name)

    def _cite_fix(self, doc_text: str, doc_name: str, unverified: List[str], meta: ProjectCodeMetadata):
        """拆分说明：实现已移至模块级 _cite_fix，此处保留方法签名做委托"""
        return _cite_fix(self, doc_text, doc_name, unverified, meta)

    def _generate_api_doc(self, project_name: str, summary: str, modules: List[WikiModule], style: str='comprehensive', meta: ProjectCodeMetadata=None):
        """拆分说明：实现已移至模块级 _generate_api_doc，此处保留方法签名做委托"""
        return _generate_api_doc(self, project_name, summary, modules, style, meta)

    def _build_wiki_index(self, project_name: str, modules: List[WikiModule], docs: List[str], result: WikiResult):
        """拆分说明：实现已移至模块级 _build_wiki_index，此处保留方法签名做委托"""
        return _build_wiki_index(self, project_name, modules, docs, result)

    def _extract_deps_info(self, summary: str):
        """拆分说明：实现已移至模块级 _extract_deps_info，此处保留方法签名做委托"""
        return _extract_deps_info(self, summary)

    def _build_front_matter(self, doc_type: str, title: str='', description: str='', tags: List[str]=None, source: str='', confidence: str='high'):
        """拆分说明：实现已移至模块级 _build_front_matter，此处保留方法签名做委托"""
        return _build_front_matter(self, doc_type, title, description, tags, source, confidence)

    def _auto_front_matter(self, name: str):
        """拆分说明：实现已移至模块级 _auto_front_matter，此处保留方法签名做委托"""
        return _auto_front_matter(self, name)

    def _anchor_evidence(self, confirmed: List[str], project_path: str, git_bin: Optional[str]=None):
        """拆分说明：实现已移至模块级 _anchor_evidence，此处保留方法签名做委托"""
        return _anchor_evidence(self, confirmed, project_path, git_bin)

    @staticmethod
    def _to_repo_rel(project_path: str, file_path: str):
        """拆分说明：实现已移至模块级 _to_repo_rel，此处保留方法签名做委托"""
        return _to_repo_rel(project_path, file_path)

    def _save_last_good(self, output_dir: str):
        """拆分说明：实现已移至模块级 _save_last_good，此处保留方法签名做委托"""
        return _save_last_good(self, output_dir)

    def _emit(self, docs: List[str], output_dir: str, name: str, content: str, front_matter: Optional[str]=None):
        """拆分说明：实现已移至模块级 _emit，此处保留方法签名做委托"""
        return _emit(self, docs, output_dir, name, content, front_matter)

    def _write_doc(self, output_dir: str, filename: str, content: str):
        """拆分说明：实现已移至模块级 _write_doc，此处保留方法签名做委托"""
        return _write_doc(self, output_dir, filename, content)

    def _setup_git_hook(self, project_path: str, output_dir: str):
        """拆分说明：实现已移至模块级 _setup_git_hook，此处保留方法签名做委托"""
        return _setup_git_hook(self, project_path, output_dir)

    def _write_agent_pointer(self, project_path: str, output_dir: str):
        """拆分说明：实现已移至模块级 _write_agent_pointer，此处保留方法签名做委托"""
        return _write_agent_pointer(self, project_path, output_dir)

    def to_report(self, result: WikiResult):
...
def generate(self, project_path: str, output_dir: str = "",
    ...
    _generate_full_pipeline / _generate_subproject_wikis / _finalize_generation
    五个模块级函数，本函数仅保留阶段编排，对外签名与行为不变。
    ...
    # 阶段 0：路径标准化 / 风格校验 / 目录准备 / LLM 可用性硬阻断
    result, project_path, project_name, output_dir, wiki_style, llm_ready = \
        _generate_prepare(self, project_path, output_dir, wiki_style)
    if not llm_ready:
        return result

    # 阶段 1：增量同步判定（R1）
    git_bin, incremental_mode, changed_files = _decide_incremental(
        self, output_dir, project_path, result)
    ...
    _finalize_generation(self, project_path, output_dir, enable_git_hook,
                         enable_agent_pointer, git_bin, result)
    return result


def _generate_prepare(self, project_path: str, output_dir: str, wiki_style: str):
    """阶段 0：路径标准化、风格校验、输出目录准备与 WikiResult 构造。

    返回 (result, project_path, project_name, output_dir, wiki_style, llm_ready)；
    llm_ready=False 表示 LLM 不可用（硬阻断），调用方应直接返回 result。
    """
    ...
    if not self.llm.is_available():
        ...
        return result, project_path, project_name, output_dir, wiki_style, False
    ...
    return result, project_path, project_name, output_dir, wiki_style, True


def _decide_incremental(self, output_dir: str, project_path: str, result: WikiResult):
    """阶段 1（R1）：增量同步判定，返回 (git_bin, incremental_mode, changed_files)。

    存在上次状态 + git 可用 + 变更文件数在阈值内 → 增量；
    ...
    return git_bin, incremental_mode, changed_files
...
def _generate_full_pipeline(self, project_path: str, project_name: str, output_dir: str,
    ...
    未发现任何 Python 模块时向 result.errors 记录并返回 None（调用方直接返回）。
    """
    # 0. 发现子项目
    subprojects = self._discover_subprojects(project_path) if include_subprojects else []
    if include_subprojects:
...
def _finalize_generation(self, project_path: str, output_dir: str,
                         enable_git_hook: bool, enable_agent_pointer: bool,
                         git_bin, result: WikiResult):
    """阶段 4：Git hook / Agent 指针（R7）/ 失败汇总 / 增量状态（R1）与 Last-good 门控（R3）。"""
    # 6. Git hook 配置
    if enable_git_hook:
...
def _discover_modules(self, project_path: str) -> List[WikiModule]:
    ...
    for entry in os.scandir(project_path):
        ...
        if entry.name in self.EXCLUDE_DIRS:
            continue
        if _is_wl_excluded(entry.path, project_path, self._wl_exclude_dirs):
            continue
        if entry.name.startswith(("Python3.", "Python2.", "pypy", ".git")):
            continue

        py_files = self._collect_py_files(entry.path, project_path)
        if py_files:
            # 加载可配置的核心模块判定规则
