...
def render_html(result: dict) -> str:
    ...
    for i, s in enumerate(steps, 1):
        ...
        rows.append(
            ...
            f"<td style='padding:10px 12px;border-bottom:1px solid #eee;font-weight:600;'>{_esc(s['keyword'])}</td>"
            ...
            f"<td style='padding:10px 12px;border-bottom:1px solid #eee;color:#555;font-size:13px;'>{_esc(ev or cand)}</td>"
            f"</tr>")
