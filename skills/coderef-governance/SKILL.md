...
| **G2 → L3 asset** | 治理产出后（③ 收尾 / ⑤ 体检后 / ⑨ 软入口档摸底） | 抽出**可复用公共工具** / 体检或摸底发现**高价值设计**（多 workflow 采用） | `coderef-asset`（L3 资产沉淀） | 成果沉淀链（见下）：是否可沉淀评估 → innovation_review 确认 → registry 归一 → asset commit 固化 |
| **G3 → 回主链/回 L1**（闭环） | L1/L3 执行后 | L1 探查发现**存量结构问题**（重复/孪生/游离）→ 转去 L2；L3 复刻落地后 → 转去 L1 | 视来源定向 | L1 发现的存量问题 → align 到 ② 差距清单立项治理（回 L2）；L3 复刻**落地后必须回 L1 `coderef-probe` 验证**（不能跳过），完整链 L1→L2→L3→L1 |
