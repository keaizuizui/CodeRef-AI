...
2. **L0→L3 逐层捋清铁律（）**：捋管线必须按自顶向下顺序走完四层——**L0 架构总览**（这是什么业务/商店还是饭店/客人是谁）→ **L1 模块盘点**（厨房/卫生间/前台/大厅有哪些模块，按层聚焦）→ **L2 模块内逻辑**（点进"方案工具"看内部 N 步流程）→ **L3 代码管线**（从模块穿透到具体代码流转/调用链）。**未捋清 L0-L3 不得进入定标（define-target）与差距分析（arch_gap）**；画布默认全量平铺是"并列展示"，必须先经层级导航（L0/L1/L2/L3 视图 + 面包屑回退）逐层捋清后，才叠加差距高亮（差距开关默认折叠）。
