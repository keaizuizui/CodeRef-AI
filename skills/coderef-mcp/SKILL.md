...
| 你的意图 | 主链入口 | 说明 |
|---|---|---|
| 单次体检 / 验证流程 / 查调用 / AI 改坏没 / 上下文丢了 | **本 skill（coderef-mcp）** | 走下方工作流 A-E |
| 存量很乱（屎山）但还拿不准值不值得系统治理，想先摸底 | **`coderef-governance`（L2 软入口档「先架构诊断」）** | 只做架构摸底＋治理价值决策门，价值成立再升级正式主链 |
| 存量结构乱（重复/孪生/模块不统一）要系统规整 | **`coderef-governance`（L2 主链）** | 先捋管线→定标→治理→验通→体检 5 阶段 |
| 每次改代码前的变更探查/回归防护/论断核验 | **`coderef-probe`（L1）** | 变更驱动的轻量探查链 |
| 治理成果想沉淀复用 / 创新识别 / 资产复刻 | **`coderef-asset`（L3）** | 资产沉淀链 |

## 场景化路由（不直面 50 工具）

> 治理主链场景化编排见 `coderef-governance` Skill（5 阶段 × 每阶段 2–5 工具）。这里给出意图 → 工具快速路由（同义词/别名 → 主工具），编程 AI 不确定用哪个工具时先查这张表。
