...
我们不把"能跑通"当验收标准，而是用多重方式审查了多类自制项目与真实开源项目，持续证明工具测得准、不误报、不撒谎：

- **错题集回归**：维护一份真实缺陷清单（错题集），在多个项目上批量跑审计，逐条核对命中是否落在真实缺陷上、是否被交叉印证、是否诚实标注"查不到"而不编造。命中率低的维度即暴露工具盲区，驱动下一轮补修。
- **端到端测试**：从项目构建知识图谱到产出审计报告，跑通完整链路，确保每个环节不短路、不丢结果。
- **正向模拟 AI 操作**：模拟编程 AI 调用各 MCP 工具（查询、审计、流程图谱、论断核验）的真实使用方式，验证工具在 AI 手里能给出确定、可复现的结论。
- **正向模拟人类操作**：模拟非编程人员验证"项目是否按预期运转"的操作路径，确保他们能拿到看得懂、可确证的结果。
