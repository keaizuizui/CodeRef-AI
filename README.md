...
审计工具最大的风险是"看起来在工作，其实没抓到真问题"。我们不把"能跑通"当验收标准，而是用三类确定性测试验证它到不到位：

- **证据审计（evidence-audit）**：在含已知缺陷的测试项目上跑工具，逐条核对命中是否落在真实缺陷上，并检查四层证据——位置是否正确、引用是否对上、是否被其他检测器交叉印证、是否诚实标注"查不到"而不编造。
- **缺陷命中率回归（defect-hit）**：注册一份真实缺陷清单，在多个真实项目上批量跑审计，统计各维度命中率。命中率低的维度即暴露工具盲区，驱动下一轮补修（跨语言、参数契约、供应链检测即是在此回归中补上的）。
- **最小复现回归**：每次修复后为具体缺陷构造最小复现用例，既验证"该报的能报出来"，也验证"不该报的不误报"，防止修一个漏一个。

通过这套流程，我们持续把"工具能跑"推进到"工具测得准、不误报、不撒谎"。
